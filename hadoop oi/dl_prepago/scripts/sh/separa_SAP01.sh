...
dir_processamento_sap01=$dir_processamento/sap01
dir_processado_sap01=$dir_processado/sap01
dir_rejeitado_sap01=$dir_rejeitado/sap01
...
/data1/etl_hdp/dl_prepago/scripts/sh/verifica_entrada_interface.sh sap01  >> $arq_log 
ParamRetCode=$?
if [ $ParamRetCode -ne 0 ] ; then
   echo "Erro $ParamRetCode ao executar verifica_entrada_interface.sh sap01" >> $arq_log
   exit ${ars_ret_code}
fi
   
arquivos=$(lista_arq $dir_processamento_sap01/SAP_NF_VENDA_${data_ref}*)

for i in $arquivos
do

  echo "Arquivo $i encontrado" >> $arq_log

  arquivo="${i##*\/}"

  # Arquivo de saida de cartao fisico
  saida_cf="$dir_processamento_sap01/${arquivo%%.*}_CF.${arquivo##*.}"

  echo "Gerando arquivo $saida_cf" >> $arq_log

  # Cartao fisico
  hadoop fs -cat $i | awk -F";" 'function ltrim(s) { sub(/^[ \t\r\n]+/, "", s); return s } function rtrim(s) { sub(/[ \t\r\n]+$/, "", s); return s } function trim(s)  { return rtrim(ltrim(s)); } BEGIN{} { if (trim($16) != "" && trim($17) != ""){ print $0 } } END{}' | hadoop fs -put - $saida_cf
  ...
  saida_ncf="$dir_processamento_sap01/${arquivo%%.*}_NCF.${arquivo##*.}"

  echo "Gerando arquivo $saida_ncf" >> $arq_log

  # Nao e cartao fisico
  hadoop fs -cat $i | awk -F";" 'function ltrim(s) { sub(/^[ \t\r\n]+/, "", s); return s } function rtrim(s) { sub(/[ \t\r\n]+$/, "", s); return s } function trim(s)  { return rtrim(ltrim(s)); } BEGIN{} { if (trim($16) == "" && trim($17) == ""){ print $0 } } END{}' | hadoop fs -put - $saida_ncf
  ...
  saida_rej="$dir_rejeitado_sap01/${arquivo%%.*}.119001"

  echo "Gerando arquivo $saida_rej" >> $arq_log

  # Rejeicao
  hadoop fs -cat $i | awk -F";" 'function ltrim(s) { sub(/^[ \t\r\n]+/, "", s); return s } function rtrim(s) { sub(/[ \t\r\n]+$/, "", s); return s } function trim(s)  { return rtrim(ltrim(s)); } BEGIN{} { if ((trim($16) != "" && trim($17) == "") || (trim($16) == "" && trim($17) != "")){ print $0 } } END{}' | hadoop fs -put - $saida_rej
  ...
  echo "Movendo o arquivo original para $dir_processado_sap01" >> $arq_log

  # Move o arquivo original para o destino
  hadoop fs -mv $i $dir_processado_sap01

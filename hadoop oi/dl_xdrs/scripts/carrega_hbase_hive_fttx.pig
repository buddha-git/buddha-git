...
hive = LOAD 'hdfs:///$dir_processamento/fttx/*' USING PigStorage('\u0001') AS (l:chararray,f:chararray,di:chararray,a:chararray,data:chararray,tl:chararray,ip:chararray,nc:chararray,lc:chararray,i6di:chararray,i6df:chararray,i6fi:chararray,i6ff:chararray,in:chararray,e:chararray,un:chararray,nu:chararray,sf:chararray,cp:chararray,s:chararray,id_sessao_md5:chararray,cdr:chararray,dt:chararray);

hive_gravar = foreach hive generate l,f,di,a,data,tl,ip,nc,lc,toHexBin(i6di) as i6di,toHexBin(i6df) as i6df,toHexBin(i6fi) as i6fi,toHexBin(i6ff) as i6ff,in,e,un,nu,sf,cp,s,id_sessao_md5,cdr,dt;

S1 = STORE hive_gravar INTO 'dl_xdrs.tmp_tb_fttx' USING org.apache.hive.hcatalog.pig.HCatStorer();
...
S2 = STORE pernaA INTO 'hbase://ns_oilegal:FTTX' USING org.apache.pig.backend.hadoop.hbase.HBaseStorage( 'p:*' );
...
S3 = STORE filteredB INTO 'hbase://ns_oilegal:FTTX' USING org.apache.pig.backend.hadoop.hbase.HBaseStorage( 'p:i' );
...
S4 = STORE filteredC INTO 'hbase://ns_oilegal:FTTX' USING org.apache.pig.backend.hadoop.hbase.HBaseStorage( 'p:i' );

-- Atualiza tabela de controle

group_layout_dt = GROUP hive BY (l, dt);

group_layout_dt_counts = FOREACH group_layout_dt GENERATE group.dt as data,COUNT(hive) AS qt_regs,group.l as layout,'fttx' as interface;

S5 = STORE group_layout_dt_counts INTO 'dl_xdrs.tmp_tb_controle_volume_fttx' USING org.apache.hive.hcatalog.pig.HCatStorer();
